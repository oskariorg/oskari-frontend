 ...
 */
{
    ...
    createModels : function() {
        ...
        if(!Ext.ClassManager.get('MapLayer')) {
    ...
    getLayersData : function() {
        ...
        for(var i = 0; i < layers.length; i++) {
            ...
            } else if(layers[i].isLayerOfType('WMTS')) {
                layer.set('type', 'wmtslayer');
    ...
    createUI : function() {
        ...
        */
        ...
        function renderActionLinks(record) {
            ...
            var layerIcon = '/framework/bundle/layerselector/images/taso.png';
            var layerIconTip = 'mapservice_maplayer_image_tooltip';

            if(record.get('type') == 'wfslayer') {
                layerIcon = '/framework/bundle/layerselector/images/database.png';
                layerIconTip = 'selected_layers_module_wfs_icon_tooltip';
            } else if(record.get('type') == 'group') {
                layerIcon = '/framework/bundle/layerselector/images/group.png';
                layerIconTip = 'mapservice_basemap_image_tooltip';
            } else if(record.get('type') == 'base') {
                layerIcon = '/framework/bundle/layerselector/images/pino.png';
                layerIconTip = 'mapservice_basemap_image_tooltip';
            }
            // layerIcon
            ...
            if(me.selected[record.get('id')]) {
                actionsHtml = actionsHtml + '<a href="#" onclick="Oskari.$().sandbox.requestByName(\'' + me.getName() + '\', \'RemoveMapLayerRequest\',[' + layerId + ']);return false;" data-qtip="' + me.sandbox.getText("mapservice_layer_delete_title") + '"><img src="' + Oskari.$().startup.imageLocation + '/framework/bundle/layerselector/images/poisto.png' + '" height="15" /></a>';
            }
            // if not selected -> add link
            else if("base" == record.get('type')) {
                actionsHtml = actionsHtml + '<a href="#" onclick="Oskari.$().sandbox.requestByName(\'' + me.getName() + '\', \'AddMapLayerRequest\',[' + layerId + ',false]);return false;" data-qtip="' + me.sandbox.getText("leftpanel_add_layer") + '"><img src="' + Oskari.$().startup.imageLocation + '/framework/bundle/layerselector/images/lisays_vihrea.png' + '" height="15" alt="add"/></a>';
            } else {
                actionsHtml = actionsHtml + '<a href="#" onclick="Oskari.$().sandbox.requestByName(\'' + me.getName() + '\', \'AddMapLayerRequest\',[' + layerId + ',true]);return false;" data-qtip="' + me.sandbox.getText("leftpanel_add_layer") + '"><img src="' + Oskari.$().startup.imageLocation + '/framework/bundle/layerselector/images/lisays_vihrea.png' + '" height="15" alt="add" /></a>';
            }

            // popup link - only show if url defined
            var dataUrl = record.get('dataUrl');
            if(dataUrl) {
                actionsHtml = actionsHtml + ' <a href="#" onclick="Oskari.$().sandbox.requestByName(\'' + me.getName() + '\', \'ShowOverlayPopupRequest\',[\'' + dataUrl + '\']);return false;" data-qtip="' + me.sandbox.getText("mapservice_layer_show_info_title") + '"><img src="' + Oskari.$().startup.imageLocation + '/framework/bundle/layerselector/images/kysymysmerkki.png' + '" height="15"/></a>';
            }
            return actionsHtml;
        };
        ...
        var searchField = Ext.create('Ext.form.TextField', {
            ...
            listeners : {
                focus : function(evt) {
                    Oskari.$().sandbox.request(me.getName(), Oskari.$().sandbox.getRequestBuilder('DisableMapKeyboardMovementRequest')());
                },
                blur : function(evt) {
                    Oskari.$().sandbox.request(me.getName(), Oskari.$().sandbox.getRequestBuilder('EnableMapKeyboardMovementRequest')());
                },
                change : {
        ...
        var layerSelectionPanel = Ext.create('Ext.panel.Panel', {
            bodyPadding : 0,
            margin : 0,
    ...
    handleMapLayerChange : function(event) {
        ...
        if(event.getOperation() === 'update') {
        	...
        	var store = me.getStore('allLayersStore');
        	var model = store.getById(layerId);
        	model.set('name', name);
        }
        else {
        	// add or remove
...
/* Inheritance *//**
 * @class Oskari.mapframework.bundle.DefaultLayerSelectorBundleInstance
 */
Oskari.clazz.define("Oskari.mapframework.bundle.DefaultLayerSelectorBundleInstance", function(b) {
...
*/
		...
		me.impl = Oskari.clazz.create('Oskari.mapframework.ui.module.layerselector.AllLayersModule');
		 ...
		 */
		var def = me.facade.appendExtensionModule(me.impl, me.name, {}/* this.impl.eventHandlers */, me, 'E', {
			'fi' : {
				title : 'Kaikki tasot'
			},
			'sv' : {

 ...
 * 
 * Module init/start/stop methods get reference to sandbox through the Oskari Module protocol.
 * TODO: documentation TBD
 */
Oskari.clazz
    .define(
	'Oskari.mapframework.sandbox.Sandbox',
	function(core) {
	    ...
	    this.maxGatheredRequestsAndEvents = 4096; // TODO: move to some conf?
...

	{
	     ...
	     *     JSON presentation of user
             * Creates Oskari.mapframework.domain.User from the given data as current user 
             */
	    setUser : function(userData) {
		/* Create a user object */
		this._user = 
		    Oskari.clazz.create('Oskari.mapframework.domain.User', 
					userData);
	    },
            /**
             * @method getUser
	    ...
	    /**
             ...
             * 
             */
	    showPopupText : function(title_key, message_key,
				     placeholders) {
		return this._core.showPopupText(title_key, message_key,
						placeholders);
	    },
            /**
             ...
             * 
             * @param {String}
             *            pBundleId bundle instance id to which the state will be mapped to 
             ...
             * Returns an object that has references to stateful components (see #registerAsStateful).
             * The objects propertynames match the instance id and property value is reference to the stateful component.
             * 
             */
		...
		this._core
		    .unregisterFrameworkComponentFromRuntimeEnvironment(
			module, module.getName());
		...
		this._core.printDebug("Sandbox is registering module '"
				      + module.getName() + "' to event '" + eventName
				      + "'");
		...
		this._core.printDebug("There are currently "
				      + oldListeners.length
				      + " listeners for event '" + eventName + "'");
		delete event;
	    },
	    registerForEventByName : function(module, eventName) {

		this._core
		    .printDebug("#*#*#* Sandbox is registering module '"
				+ module.getName()
				+ "' to event '"
				+ eventName + "'");
		var oldListeners = this._listeners[eventName];
		...
		var eventName = event.getName();
		this._core
		    .printDebug("Sandbox is unregistering module '"
				+ module.getName() + "' from event '"
				+ eventName + "'");
		var oldListeners = this._listeners[eventName];
		if (oldListeners == null) {
		    // return no listeners
		...
		if (deleteIndex > -1) {
		    ...
		    this._core
			.printDebug("Module unregistered successfully from event");
		} else {
		    this._core
			.printDebug("Module does not listen to that event, skipping.");
		}
		...
		if( !this.debugRequests)
		    return;
		var reqLog = { from: creator, reqName: req.getName() };
		...
		if (creator.getName != null) {
			...
			.findRegisteredModule(creator.getName());
		} else {
		    creatorComponent = this
			.findRegisteredModule(creator);
		}

		if (creatorComponent == null) {
		    throw "Attempt to create request with unknown component '"
			+ creator + "' as creator";
		}

		this._core.setObjectCreator(request, creatorComponent);

		this.printDebug("Module '" + creatorComponent
				+ "' is requesting for '"
				+ this.getObjectName(request) + "'...");
		...
		this.printDebug("#!#!#! --------------> requestByName "
				+ requestName);
		var requestBuilder = this
		    .getRequestBuilder(requestName);
		var request = requestBuilder.apply(this, requestArgs);

		var creatorComponent = null;
		if (creator.getName != null) {
		    creatorComponent = 
			this.findRegisteredModule(creator.getName());
		} else {
		    creatorComponent = 
			this.findRegisteredModule(creator);
		}
		...
		window.setTimeout(function() {
		    me.printDebug("#!#!#! POSTING --------------> requestByName "
				  + requestName);
		    var requestBuilder = me
			.getRequestBuilder(requestName);
		    ...
		    me.printDebug("Module '" + creatorComponent
				  + "' is POSTING a request for '"
		...
		if( !retainEvent ) {
		    ...
		    this._core
			.printDebug("Sandbox received notifyall for event '"
				    + eventName + "'");
		    /*
		...
		if (modules != null) {
		    if( !retainEvent ) {
			this._core.printDebug("Found " + modules.length
					      + " interested modules");
		    }
		    for ( var i = 0; i < modules.length; i++) {
			var module = modules[i];
			if( !retainEvent ) {
			    this._core.printDebug("Notifying module '"
						  + module.getName() + "'.");
		    ...
		    if( !retainEvent ) {
			this._core
			    .printDebug("No interested modules found.");
		    }
		}
		...
		return this._modulesByName[name] ?
		    this._modulesByName[name].getName() : null;
		...
		return this._core.copyObjectCreatorToFrom(objTo,
							  objFrom);
	    },

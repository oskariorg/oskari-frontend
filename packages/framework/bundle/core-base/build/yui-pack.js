 ...
 * A protocol class for registered request handlers. You need to implement a class 
 ...
 * TODO: Move handlers (and events as well as requests) to handler bundles with registrable handlers
 */
Oskari.clazz.define(
                ...
                function() {
                    ...
                    this._sandbox = Oskari.clazz.create(
                                'Oskari.mapframework.sandbox.Sandbox', this);
                ...
                /* prototype */
                {
                     ...
                     * Inits core
                     * 
                     * @param {Oskari.mapframework.ui.manager.mapfull.MapFullUiManager/Object} uiManager
                     *            that builds all needed modules and rest of ui
                     * TODO: uiManager should be interfaced (possibly needs only createModulesAndUi(sandbox))
                     ...
                     *            map before starting
                     * @param {Object} allLayersJson
                     *            all layers in JSON format
                     * @param userInterfaceLanguage
                     *            current language (NOT USED ANYMORE) - deprecated
                     * @param mapPublisherWizardUrl
                     *            if url is given this will take us to map
                     *            publisher
                     * @param {Boolean} doSniffing
                     *            true if we should log usage statistics
                     */
                    init : function(services, enhancements) {
                        ...
                        if (services) {
                            for ( var s = 0; s < services.length; s++) {
                                this._servicesByQName[services[s].getQName()] = services[s];
                            }
                        }
                        ...
                        for ( var i = 0; i < services.length; i++) {
                            this.registerFrameworkComponentToRuntimeEnvironment(
                                            services[i], services[i].getName());
                        }

                        /* build up domain */
                        this.printDebug("Sandbox ready, building up domain...");
                        
                        this.createMapDomain();
                    ...
                    start : function() {
                        ...
                        if (this._doSniffing) {
                            ...
                            var snifferService = this
                                    .getService('Oskari.mapframework.service.UsageSnifferService');
                        	if(snifferService) {
                    ...
                    defaultRequestHandlers : {
                        ...
                        'DeactivateAllOpenlayersMapControlsButNotMeasureToolsRequest' : function(
                                request) {
                            this.handleDeactivateAllOpenlayersMapControlsButNotMeasureToolsRequest(request);
                            return true;
                        },
                        'DeactivateAllOpenlayersMapControlsRequest' : function(
                                request) {
                        ...
                        'HighlightWFSFeatureRequestByGeoPoint' : function(
                                request) {
                            this.handleHighlightWFSFeatureRequestByGeoPoint(request);
                            return true;
                        },
                    ...
                    processRequest : function(request) {
                        ...
                        if (handlerFunc) {
                            ...
                            if (handlerClsInstance) {
                                 ...
                                 * .handleRequest(core)
                                 */
                                rv = handlerClsInstance.handleRequest(this,
                                        request);
                     ...
                     * one handler / request ?
                     */
                    addRequestHandler : function(requestName,
                            handlerClsInstance) {
                     ...
                     * one handler / request ?
                     */
                    removeRequestHandler : function(requestName,
                            handlerInstance) {
                    ...
                    getQNameForRequest : function(name) {
                        ...
                        if (!qname) {
                            this
                                    .printDebug("#!#!# ! Updating request metadata...");
                            var allRequests = Oskari.clazz
                            ...
                            this
                                    .printDebug("#!#!# ! Finished Updating request metadata...");
                    ...
                    getQNameForEvent : function(name) {
                        ...
                        if (!qname) {
                            this
                                    .printDebug("#!#!# ! Updating event metadata...");
                            ...
                            this
                                    .printDebug("#!#!# ! Finished Updating event metadata...");
                     ...
                     *            name
                     */
                    registerFrameworkComponentToRuntimeEnvironment : function(
                            component, name) {
                        this.printDebug("registering framework component '"
                                + name + "' to runtime");
                        Oskari.$().mapframework.runtime.components[name] = component;

                    },

                    unregisterFrameworkComponentFromRuntimeEnvironment : function(
                            component, name) {
                        this.printDebug("unregistering framework component '"
                                + name + "' from runtime");
                    ...
                    checkArrayIfTheseAreSame : function(checkedArray) {
                        if (checkedArray.length > 1) {
                            ...
                            for (var i = 1; i < checkedArray.length; i++) {
                                if (checkedArray[i] != firstArrayItem
                                        && checkedArray[i] != null) {
                                    return false;
                                }
                            }
                    ...
                    getLanguage : function() {
                        return Oskari.getLang(); //this._languageService.getLanguage();
                    },
                    ...
                    getBrowserWindowSize : function() {
                        ...
                        var size = {};
                        size.height = jQuery(window).height(); // height;
                        size.width = width;

                        this.printDebug("Got browser window size is: width: "

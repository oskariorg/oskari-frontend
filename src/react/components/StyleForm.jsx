...
const previewPlaceholderIcon = (format, fillColor, strokeColor, strokeWidth) => {
    switch(format) {
        case 'line':
            return (<svg viewBox="0 0 50 50" width="50" height="50" xmlns="http://www.w3.org/2000/svg"><path fill="none" stroke="#000000" d="M10,15L20,35L40,25" stroke-width="1" stroke-linejoin="miter" stroke-linecap="butt" stroke-dasharray="0"></path></svg>);
        case 'point':
...
export class StyleForm extends React.Component {
    ...
    _createPreview (fillColor, strokeColor, strokeWidth) {
        return (
            <Preview previewIcon={ previewPlaceholderIcon( this.state.format, fillColor, strokeColor, strokeWidth ) } />
        );
    }
            ...
            <Card>
                ...
                <Row>
                    <Form.Item name='dotSizeControl' label='Size' initialValue={ 3 } { ...formLayout }>
                        <InputNumber min={ 1 } max={ 5 } formatter={ sizeFormatter } parser={ sizeFormatter } />
                    </Form.Item>
                </Row>

                <Row>
                    { this._createPreview(this.state.dotFillColor, this.state.dotColor) }
                </Row>
            </Card>
        );
    }
    ...
    _getLineTab () {
        return (
            <Card>
                ...
                <Row>
                    { this._createPreview(this.state.lineColor) }
                </Row>
            </Card>
        );
    }
    ...
    _getAreaTab () {
        return (
            <Card>
                ...
                <Row>
                    { this._createPreview(this.state.areaFillColor, this.state.areaLineColor) }
                </Row>
            </Card>
        );
    }

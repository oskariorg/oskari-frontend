...
function() {
    ...
    _addMapLayerToMap : function(layer, keepLayerOnTop, isBaseMap) {
        ...
        for (var i = 0, ilen = layers.length; i < ilen; i++) {
            ...
            var layerImpl = new Cesium.ImageryLayer(new Cesium.WebMapServiceImageryProvider({
                url: wms.URL,
                layers : wms.LAYERS,
                parameters: { format : wms.FORMAT }
            }));

            this.mapModule.addLayer(layerImpl, _layer, layerIdPrefix + _layer.getId());

            this._sandbox.printDebug("#!#! CREATED Cesium.WebMapServiceImageryProvider for " + _layer.getId());

...
const getContent = (state, controller, onClose, renderDescriptionPopup) => {
    const { showAsBanner, currentBanner } = state;
    const announcement = showAsBanner[currentBanner - 1];
    ...
    const isLast = currentBanner === showAsBanner.length;
    ...
    const action = <ActionContainer>
        ...
        {showAsBanner.length > 1 && (
            <ButtonContainer>
                <StyledPagination current={currentBanner} total={showAsBanner.length} defaultPageSize={1} hideOnSinglePage onChange={onPageChange} itemRender={itemRender}/>
    ...
    return {
        ...
        action,
        closable: showAsBanner.length > 1 ? false : true,
        description,
        showIcon: true,

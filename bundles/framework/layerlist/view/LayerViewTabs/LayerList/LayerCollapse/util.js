 ...
 */
export const groupLayersAdmin = (layers, method, tools, allGroups = [], allDataProviders = [], noGroupTitle, isUserAdmin) => {
    const groupList = [];
    let group = null;
...
export const groupLayers = (layers, method, tools, allGroups = [], allDataProviders = [], noGroupTitle, isUserAdmin) => {
    ...
    const recursive = function(group) {
        if (group.hasSubgroups()) {
            ...
            console.log("Check for undefineds");
            console.log(testt);
            const filtered = testt.filter(t => t !== undefined)
            console.log(filtered);
            filtered.length > 0 && newGroup.setGroups(filtered)
            console.log();
            filtered && newGroup.setGroups(filtered)
            return newGroup;
        } else {
            return;
        }

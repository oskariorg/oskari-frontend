/**
 * @class Oskari.mapframework.bundle.parcel2.CategoryHandler
 * 
 * Handles category related functionality for my places (map layers etc)
 */
Oskari.clazz.define("Oskari.mapframework.bundle.parcel2.CategoryHandler",
...
function(instance) {
    ...
    eventHandlers : {
        /**
         * @method Parcel.ParcelChangedEvent
         * Checks if categories have been changed and updates corresponding maplayers accordingly
         * @param {Oskari.mapframework.parcel.event.ParcelChangedEvent} event
         */
        'Parcel.ParcelChangedEvent' : function(event) {
    ...
    _handlePlacesChanged : function() {
        ...
        for(var catIdx = 0; catIdx < categories.length; ++catIdx) {
            ...
            if(!found) {
                ...
                var parcelLayer = mapLayerService.createMapLayer(json);
                mapLayerService.addLayer(parcelLayer);
            }
            //this.uiItems.gridPanel.addOrUpdateCategory(cat);
        }
        
        if(this.initialLoad) {
            // add the parcel layers programmatically since normal link processing 
    ...
    _getMapLayerId : function(categoryId) {
        if(!categoryId) {
            ...
            var defCat = this.parcelService.getDefaultCategory();
            if(defCat) {
    ...
    _getMapLayerJson : function(categoryModel) {
        ...
        //baseJson.wmsUrl = "/karttatiili/parcel?myCat=" + categoryModel.getId() + "&";
    ...
    editCategory : function(category) {
        ...
        var form = Oskari.clazz.create('Oskari.mapframework.bundle.parcel2.view.CategoryForm', me.instance);
        var values = {
    ...
    getCategoryFromFormValues : function(values) {
        var category = Oskari.clazz.create('Oskari.mapframework.bundle.parcel2.model.ParcelCategory');

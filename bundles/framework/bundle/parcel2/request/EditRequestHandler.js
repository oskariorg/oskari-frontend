/**
 * @class Oskari.mapframework.bundle.parcel2.request.EditRequestHandler
 * Handles sequests for a saved "my place" or my places categorires to be opened for editing
 */
Oskari.clazz.define('Oskari.mapframework.bundle.parcel2.request.EditRequestHandler', 
 ...
 * @param {Oskari.mapframework.bundle.parcel2.ParcelBundleInstance} instance 
...
function(sandbox, instance) {
     ...
     * @param {Oskari.mapframework.bundle.parcel2.request.EditPlaceRequest/Oskari.mapframework.bundle.parcel2.request.EditCategoryRequest} request
     *      request to handle
     */
    ...
    _handleEditPlace : function(sandbox, request) {
        this.sandbox.printDebug("[Oskari.mapframework.bundle.parcel2.request.EditRequestHandler] edit requested for place " + request.getId());
    ...
    _handleEditCategory : function(sandbox, request) {
        this.sandbox.printDebug("[Oskari.mapframework.bundle.parcel2.request.EditRequestHandler] edit requested for category " + request.getId());
        var service = this.instance.getService();
    ...
    _handleDeleteCategory : function(sandbox, request) {
        this.sandbox.printDebug("[Oskari.mapframework.bundle.parcel2.request.EditRequestHandler] delete requested for category " + request.getId());
        var service = this.instance.getService();
        var category = service.findCategory(request.getId());
        if(category) {
    ...
    _handlePublishCategory : function(sandbox, request) {
        this.sandbox.printDebug("[Oskari.mapframework.bundle.parcel2.request.EditRequestHandler] (un/)publish requested for category " + request.getId());
        var service = this.instance.getService();
        var category = service.findCategory(request.getId());
        if(category) {

 ...
 */
Oskari.clazz
  .define('Oskari.mapframework.bundle.search.Flyout', 
	  ...
	  function(instance) {
	    ...
	    startPlugin : function() {
	      this.template = 
		jQuery('<div class="searchContainer">' + 
		       '<div class="controls">' +
		       ...
		       '</div>');
	      this.templateResultTable = 
		jQuery('<table class="search_result">' + 
		       '<thead><tr></tr></thead>' + 
		       '<tbody></tbody>' + 
		       '</table>');
	      this.templateResultTableHeader = 
		jQuery('<th><a href="JavaScript:void(0);"></a></th>');
	      
	      this.templateResultTableRow = 
		jQuery('<tr>' + 
		...
		resultList.append('searchservice_search_alert_title: ' + 
				  result.errorText);
		return;
		...
		// close flyout
		inst.sandbox
		  .postRequestByName('userinterface.UpdateExtensionRequest', 
				     [ me.instance, 'close' ]);
	      }
	      // render results
		...
		return function() {
		  ...
		  var headerContainer = 
		    tableHeaderRow.find('a:contains(' + 
					scopedValue.title + 
					')');
		...
		tableHeaderRow.append(header);
	      ...
	      resultList.append('<div><h3>Tulokset:' + 
				result.totalCount + 
				' hakutulosta haulla ' + 
				searchKey + 
				'</h3></div>');

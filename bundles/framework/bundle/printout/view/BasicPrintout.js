...
function(instance, localization, backendConfiguration) {
    ...
    _createSizePanel : function() {
        ...
        for(var i = 0; i < this.sizeOptions.length; ++i) {
            ...
            toolContainer.find('label').append(label).attr({'for': option.id, 'class': 'printout_radiolabel'});
            if(option.selected) {
                toolContainer.find('input').attr('checked', 'checked');
            }
            contentPanel.append(toolContainer);
            toolContainer.find('input').attr({'value': option.id, 'name': 'size', 'id': option.id});
    ...
    _createSettingsPanel : function() {
        ...
        for(var i = 0; i < this.formatOptions.length; ++i) {
            ...
            toolContainer.find('input').attr({'value': option.format, 'name': 'format', 'id': option.id});
            toolContainer.find('input').change(closureMagic(option));
        }
        ...
        mapTitle.find('.printout_title_field').attr({'value': '', 'placeholder': this.loc.mapTitle.label});

        var logo = this.templateLogo.clone();
        logo.find('.printout_logo_check').attr({});
        logo.find('input').attr({'id': 'printout_logo', 'checked': 'checked'});
        logo.find('label').html(this.loc.mapLogo.label).attr({'for': 'printout_logo', 'class': 'printout_checklabel'});

        var scale = this.templateScale.clone();
        scale.find('.printout_scale_check').attr({'id': 'printout_scale'});
        scale.find('input').attr({'id': 'printout_scale', 'checked': 'checked'});
        scale.find('label').html(this.loc.mapScale.label).attr({'for': 'printout_scale', 'class': 'printout_checklabel'});

        var date = this.templateLogo.clone();
        date.find('.printout_date_check').attr({'id': 'printout_date'});
        date.find('input').attr({'id': 'printout_date', 'checked': 'checked'});
        date.find('label').html(this.loc.mapDate.label).attr({'for': 'printout_date', 'class': 'printout_checklabel'});

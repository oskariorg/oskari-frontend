...
function(config, plugin) {
    ...
    this.rootURL = location.protocol + "//" + this.config.hostname + ":" + this.config.port + this.config.contextPath;
...
Oskari.clazz.category('Oskari.mapframework.bundle.mapwfs2.service.Mediator', 'getters', {
    ...
    getWFSFeature : function(data) {
        ...
        var event = this.plugin.getSandbox().getEventBuilder("WFSFeatureEvent")(layer, data.data.feature);
        this.plugin.getSandbox().notifyAll(event);
    },

    ...
    function (config) {
        ...
        mapMoveHandler: function () {
            ...
            if (this.zoomLevel !== zoom) {
                ...
                var x,
                    j,
                    fids;
                for (x = 0; x < this.activeHighlightLayers.length; ++x) {
        ...
        showInfoBox: function () {
            ...
            for (di = 0; di < data.fragments.length; di++) {
                ...
                contentWrapper = this.template.wrapper.clone();

                headerWrapper = this.template.getinfo_result_header.clone();
                titleWrapper = this.template.getinfo_result_header_title.clone();
        ...
        formatWFSFeaturesForInfoBox: function (wfsLayers) {
            ...
            for (x = 0; x < wfsLayers.length; x++) {
                ...
                var i,
                    j,
                    k;
                for (i = 0; i < wfsLayers[x].features.length; i++) {
                    feature = {};
        ...
        getGrid: function () {
            ...
            if (OLGrid) {
                ...
                var iRow,
                    ...
                    clen,
                    bounds;
                for (iRow = 0, len = OLGrid.length; iRow < len; iRow++) {
                    row = OLGrid[iRow];
                    for (iCol = 0, clen = row.length; iCol < clen; iCol++) {
                        tile = row[iCol];

                        // if failed grid
                        if (tile.bounds.left === undefined || tile.bounds.left === null ||
                                tile.bounds.bottom === undefined || tile.bounds.bottom === null ||
                                tile.bounds.right === undefined || tile.bounds.right === null ||
                                tile.bounds.top === undefined || tile.bounds.top === null) {
                            return null;
                        }
        ...
        setPrintTile: function (layer, bbox, imageUrl) {
            if (this._printTiles[layer.getId()] === null || this._printTiles[layer.getId()] === undefined) {
                this._printTiles[layer.getId()] = [];
            }
        ...
        getNonCachedGrid: function (layerId, grid) {
            ...
            var result = [],
                ...
                bboxKey,
                dataForTile;
            for (i = 0; i < grid.bounds.length; i++) {
        ...
        getAllFeatureIds: function (layer) {
            var fids = layer.getClickedFeatureIds().slice(0),
                k;
            for (k = 0; k < layer.getSelectedFeatures().length; ++k) {

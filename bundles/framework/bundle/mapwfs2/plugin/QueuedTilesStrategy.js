    ...
    function (options) {
        ...
        flushTileQueue: function () {
            ...
            for (n = 0; n < q.length; n++) {
                if (q[n].tileFeature !== null && q[n].tileFeature !== undefined) {
        ...
        triggerRead: function () {
            ...
            for (r = 0; r < gridGrid.length; r++) {
                for (c = 0; c < gridGrid[r].length; c++) {
                    bs = gridGrid[r][c].bounds.clone();
                    tileBounds = {
                        left: bs.left,
                        top: bs.top,
                        right: bs.right,
                    ...
                    if (debugGridFeatures) {
                        ptFromA = new OpenLayers.Geometry.Point(bs.left, bs.bottom);
                        ptToA = new OpenLayers.Geometry.Point(bs.right, bs.top);
                        ptFromB = new OpenLayers.Geometry.Point(bs.left, bs.top);
                        ptToB = new OpenLayers.Geometry.Point(bs.right, bs.bottom);
                        boundGeomArea = new OpenLayers.Geometry.LineString([ptFromA, ptToB, ptToA, ptFromB, ptFromA]);
                        boundsFeature = new OpenLayers.Feature.Vector(
                            boundGeomArea,
                            {

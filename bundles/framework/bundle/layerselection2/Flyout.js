    ...
    .define('Oskari.mapframework.bundle.layerselection2.Flyout',
		...
		startPlugin : function() {
			...
			jQuery('<div class="left-tools">' + 
			       ...
			       '</a>' + 
			       '&nbsp;' + 
			       '<span class="temphidden" style="display: none;">' + 
			       loc['hidden'] + 
			       '</span>' + 
			       '</div>' + 
		...
		handleLayerVisibilityChanged : function(layer, isInScale, isGeometryMatch) {
		    ...
		    var layerDiv = 
			jQuery(this.container).find('li.layer.selected[layer_id=' + 
						    layer.getId() + ']');
		    var loc = this.instance.getLocalization('layer');
		    ...
		    this._appendLayerFooter(layerDiv, 
					    layer, 
					    isInScale, 
					    isGeometryMatch);
		},
		...
		_layerOpacityChanged : function(layer, newOpacity) {
			...
			jQuery(this.container).find('li.layer.selected[layer_id=' + 
						    layer.getId() + ']');
		    layerDiv.find('div.layer-opacity input.opacity')
		...
		_createLayerFooter : function(layer, layerDiv) {
		    ...
		    */
			...
			if (publishPermission == 'can_be_published') {
				...
				tools.find('div.layer-rights')
				    .html(loc.rights['login-url']);
			    } else {
				tools.find('div.layer-rights')
				    .html('<a href="/web/fi/login">login</a>');
			    }
			    if (loc.rights && loc.rights['need-login']) {
				tools.find('div.layer-rights')
				    .attr("title", loc.rights['need-login']);
			    } else {
				tools.find('div.layer-rights')
				    .attr("title", 'Login required');
			    }
			    
			} else {

			    if (loc.rights &&
				loc.rights['no_publication_permission'] &&
				loc.rights['no_publication_permission'].label) {
				tools.find('div.layer-rights')
				    .html(loc.rights['no_publication_permission'].label);
			    } else {		    
				tools.find('div.layer-rights')
				    .html('Publication not permitted');
			    }
			    if (loc.rights &&
				loc.rights['no_publication_permission'] &&
				loc.rights['no_publication_permission'].tooltip) {
				tools.find('div.layer-rights')
				    .attr("title",
					  loc.rights['no_publication_permission'].tooltip);
			    } else {
				tools.find('div.layer-rights')
				    .attr("title", 'Publication not permitted.');
			    }		    
			}
			...
			if (loc.rights &&
			    loc.rights[publishPermission] &&
			    loc.rights[publishPermission].label) {
			    tools.find('div.layer-rights')
				.html(loc.rights[publishPermission].label);
			} else {
			    tools.find('div.layer-rights')
				.html(publishPermission);		  
			}

			if (loc.rights &&
			    loc.rights[publishPermission] &&
			    loc.rights[publishPermission].tooltip) {
			    tools.find('div.layer-rights')
				.attr("title",loc.rights[publishPermission].tooltip);
			} else {
			    tools.find('div.layer-rights')
				.attr("title", publishPermission);
			}
		    }
			...
			tools.find('div.layer-description a')
			    .attr('href', layer.getDataUrl());
			tools.find('div.layer-description a')
			    .attr('target', '_blank');
		    }
		 ...
		 */
		handleLayerSelectionChanged : function(layer, 
						       isSelected, 
						       keepLayersOrder) {
		    // add layer
		    if (isSelected == true) {
			...
			var layerDiv = 
			    jQuery(this.container).find('li[layer_id=' + 
							layer.getId() + ']');
			if (layerDiv) {
			    layerDiv.remove();
			}

 ...
 * mapframework
 *
 * @to-do - fix preloaded mode processing; replace empty.js hacks with source state handling 
...
Oskari = (function() {
	...
	/** template for loader */
	var module_loader = function(requireImpl) {
		this.requireImpl = requireImpl;
	};
	module_loader.prototype = {
		"require" : function(list,cb) {
			this.requireImpl(list,cb);	
		}
	}
	...
	bundle_loader.prototype = {
		...
		"addModules" : function(modulesDefinition) {
			this.hasModules = true;
			for(p in modulesDefinition ) {
				this.modules[p] = modulesDefinition[p];
				this.modulesLoaded[p] = false;
			}
		},
		"getState" : function() {
		...
		"isFinished" : function() {
			var filesState = false;
			var modulesState = true;

			var numFiles = this.filesRequested;
			var filesLoaded = this.filesLoaded;
			filesState = numFiles == 0 || numFiles == filesLoaded;

			for(p in this.modulesLoaded ) {
				modulesState = modulesState && this.modulesLoaded[p];
			}
			/*console.log("bundle_loader: ",this.hasModules,filesState, modulesState);*/

			return filesState && modulesState;
		},
		/**
		 * @method commit
		 *
		...
		"commit" : function() {
			...
			/* module loaders */
			for( p in me.modules ) {
				module_loaders[p].require(me.modules[p], function() {
					me.modulesLoaded[p] = true;
					if( me.isFinished() ) {
						me.callback();
					}
				});	
	...
	bundle_manager.prototype = {
		...
		"install" : function(implid, bp, srcs, metadata) {
			...
			me.modules[bundleImpl] = metadata.modules ;
		...
		"loadBundleSources" : function(implid) {
			...
			var mods = me.modules[bundleImpl];
			if( mods  ) {
				bl.addModules(mods);
			}
	...
	var bndl = {
		...
		/*
		 * @static
		 * @method Oskari.registerModuleLoader
		 * 
		 * registers module loader f.ex. Oskari.registerModuleLoader('dojo', new dojo_module_loader(require))
		 */
		registerModuleLoader: function(key, moduleLoaderInstance ) {
			module_loaders[key] = moduleLoaderInstance ;
		},

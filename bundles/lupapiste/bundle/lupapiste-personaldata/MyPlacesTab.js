...
function(instance, localization) {
    ...
    eventHandlers : {
        ...
        'LupaPisteMyPlaces.MyPlacesChangedEvent' : function(event) {
            var service = this.instance.sandbox.getService('Oskari.lupapiste.bundle.myplaces2.service.MyPlacesService');
...
/*        var layerId = 'myplaces_' + categoryId; 
    ...
    _editPlace : function(data) {
        ...
        // close this Flyout
        var request = this.instance.sandbox.getRequestBuilder('userinterface.UpdateExtensionRequest')(this.instance, 'close', this.instance.getName());
        this.instance.sandbox.request(this.instance.getName(), request); 
        // request form
        request = this.instance.sandbox.getRequestBuilder('LupaPisteMyPlaces.EditPlaceRequest')(data.id);
        this.instance.sandbox.request(this.instance, request);
    },
    /**
     * @method _deletePlace
    ...
    _createCategoryTab : function(category) {
        ...
        var visibleFields = ['name', 'desc', 'height', 'area', 'length', 'estates', 'delete', 'edit']; //, 'createDate', 'updateDate'];
        ...
        var estatesRenderer = function(name, data) {
          var link = me.linkTemplate.clone();
          link.append(name);
          link.bind('click', function() {
            var loc = me.instance.getLocalization();
            var dialog = Oskari.clazz.create('Oskari.userinterface.component.Popup');
            var okBtn = dialog.createCloseButton(loc.button.close);
            var content = jQuery('<div id="estatestable" style="max-height:300px; overflow-y:scroll; overflow-x:none;"></div>');
            content.append('<img src="' + Oskari.app.appConfig.lupakartta.conf.ajaxloader +'" />');
            dialog.show(loc.tabs.myplaces.grid.estates, content, [okBtn]);
            jQuery.ajax({       
              url: Oskari.app.appConfig.lupakartta.conf.ajaxurl + "/property-info-by-wkt",
              data: {"wkt": data.geometry.toString()},
              dataType: "json",
              success: function(data) {
                function formatKiinttunnus(tunnus) {
                  var a = tunnus.substring(0,3).replace(/^0+/, '');
                  var b = tunnus.substring(3,6).replace(/^0+/, '');
                  var c = tunnus.substring(6,10).replace(/^0+/, '');
                  var d = tunnus.substring(10,14).replace(/^0+/, '');
                ...
                content.empty();
                var table = jQuery('<table><thead><tr><th>' + loc.tabs.myplaces.estate + '</th><th>' + loc.tabs.myplaces.type + '</th></tr></thead></table>');
                content.append(table);
                for(var i = 0; i < data.length; ++i) {
                  table.append('<tr><th>' + formatKiinttunnus(data[i].kiinttunnus) + '</th><th>' + data[i].rekisteriyksikkolaji.selite[Oskari.getLang()] + '</th><tr>');
                }
              }
            });
            return false;
        ...
        panel.grid.setColumnValueRenderer('estates', estatesRenderer);

...
Oskari.clazz.define(
    ...
    function (config, locale) {
        ...
        me.colorsets_seq = null;
        me.colorsets_qual = null;
        me.content = null;
        me.dialog = null;
        me.colorsetIndex = 0;
        me.currentColorSet = 'seq';
        me.curCol = null;
        me._layer = null;
        me._params = null;
        me.minClassNum = 2;
        me.maxClassNum = 9;
        me.numberOfClasses = 5;
        me.isSelectHilightedMode = false;
        ...
        _initState: function () {
            ...
            if (!state.methodId || state.methodId < 1) {
                state.methodId = '1';
            }

            if (state.cmode === null || state.cmode === undefined) {
                state.cmode = '';
            }

            if (state.numberOfClasses === null || state.numberOfClasses === undefined) {
                state.numberOfClasses = 5;
            } else if (state.numberOfClasses < 3) {
                state.numberOfClasses = 2;
            }

            if (state.filterMethod === null || state.filterMethod === undefined) {
                state.filterMethod = '';
            }

            if (state.filterInput === null || state.filterInput === undefined) {
                state.filterInput = [];
            }

            if (state.filterRegion === null || state.filterRegion === undefined) {
                state.filterRegion = [];
            }

            if (state.municipalities === null || state.municipalities === undefined) {
        ...
        classifyData: function (event) {
            ...
            if (params.COL_VALUES.length < 2) {
                ...
                classify = el.find('.classifications');
                classify.find('.block').remove();
                block = jQuery(me.templates.block);
                block.append(
                    jQuery(me.templates.classificationGuide).text(
                        this._locale.select4Municipalities
                    )
                );
                classify.append(block);
                // Show legend in content
                el.find('div.content').show();
            }
            ...
            if (me._hasNonNumericValues(params.COL_VALUES)) {
                ...
                isNonNumeric = false;
                this._showClassificationOptions(el);
            }
            ...
            if (isNonNumeric) {
                ...
                for (k = 0, dataLen = col_data.length; k < dataLen; k += 1) {
            ...
            if (params.COL_VALUES.length >= 2) {
                var legendRounder = function (i) {
                    ...
                    colortab = gstats.getHtmlLegend(null, sortcol.name, true, legendRounder, classificationMode);
                classify = el.find('.classifications');
                classify.find('.block').remove();
        ...
        refresh: function (element) {
            ...
            header.append(me._locale.classify.classify);
            el.append(header);
            ...
            var content = jQuery('<div class="content"></div>'),
                ...
                methods = [
                    me._locale.classify.jenks,
                    me._locale.classify.quantile,
                    me._locale.classify.eqinterval,
                    me._locale.classify.manual
                ],
                i,
                opt;
            ...
            if (me._state.allowClassification !== false) {
                classifyOptions
                    ...
                    .append(me._locale.classify.classifymethod + '<br><select class="method"></select><br>');
                ...
                var classcnt = classifyOptions.find(
                    'div.classCount div.countSlider'
                );
                classcnt
                    .append(me._locale.classify.classes)
                ...
                manualcls
                    .append('<input type="text" name="breaksInput" placeholder="' + me._locale.classify.manualPlaceholder + '"></input>')
                ...
                var modes = ['distinct',
                    ...
                    modeSelect =
                    modeSelector.find('select.classification-mode');
                ...
                var colorsButton = jQuery(
                    '<input type="button" value="' +
                    me._locale.colorset.button +
                    '" />'
                );
                ...
                classifyOptions.find('div.classificationMode').append(
                    modeSelector
                );
        ...
        _getColors: function (colorset, colorindex, classind) {
            if (!this.colorsets_div) {
                throw Error('_getColors(): missing this.colorsets_div.');
            }
            if (!this.colorsets_qual) {
                throw Error('_getColors(): missing this.colorsets_qual.');
            }
            if (!this.colorsets_seq) {
                throw Error('_getColors(): missing this.colorsets_seq.');
            }
            var colors = null;
            if (colorset === 'div') {
        ...
        _createColorDialog: function () {
            ...
            opt = jQuery(
                '<option value="qual">' +
                this._locale.colorset.qualitative +
                '</option>'
            );
            sel.append(opt);
            opt = jQuery(
                '<option value="div">' +
                this._locale.colorset.divergent +
                '</option>'
            );
            sel.append(opt);
            sel.change(function (e) {
        ...
        _createColorTable: function () {
            ...
            for (i = 0; i < rows; i += 1) {
                ...
                for (j = 0; j < columns; j += 1) {
                    ...
                    tableCell.css('background-color',
                        '#' + colorArr[j]);
        ...
        _hiliSelectedColors: function () {
            var me = this,
                rows; // FIXME rows isn't used for anything
        ...
        _colorTableChanged: function () {
            ...
            classcnt.find('#slider-range-max').slider({
                range: 'min',
                min: cl_min,
                max: cl_max,
                value: curcla,
        ...
        _adjustClassificationSlider: function (checkedItemsCount) {
            ...
            var slider = jQuery('#slider-range-max'),
                selectedVal = slider.slider('option',
                    'value'),
                sliderSettings = this._checkClassCount(checkedItemsCount, selectedVal),
            ...
            // Disable the slider if the only allowed value is 2?
            if (max < 3) {
                slider.slider('option',
                    'disabled', true);
            } else {
        ...
        _checkClassCount: function (checkedItemsCount, selectedVal) {
            ...
            var min = 2,
                max = checkedItemsCount < this.maxClassNum ? checkedItemsCount > 2 ? checkedItemsCount : 3 : this.maxClassNum;

            max -= 1; // Why?
            var val = Math.max(min, Math.min(selectedVal, max));
            return {
                'min': min,
                'max': max,
                'value': val
         ...
         *
         */
        _loadStateVariables: function (element) {
            var me = this,
                state = this._state,
                el = element || me.getElement();
            ...
            if (me._state.allowClassification !== false) {
                ...
                if (state.classificationMode) {
                    var modeSelect = el.find('.classification-mode');
                ...
                if (state.methodId !== null && state.methodId !== undefined && state.methodId > 0) {
                    var select = el.find('.classificationMethod').find('.method');
                    select.val(state.methodId);
                    // The manual breaks method:
                    if (state.methodId === '4' && state.manualBreaksInput) {
                        var manualInput = el.find('.manualBreaks').find('input[name=breaksInput]');
                        manualInput.val(state.manualBreaksInput);
                        el.find('.countSlider').hide();
                        el.find('.manualBreaks').show();
                    }
                }

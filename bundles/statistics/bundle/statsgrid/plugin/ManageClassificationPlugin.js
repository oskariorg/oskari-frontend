 ...
 *
 * This is a plugin to classify and manage slickgrid thematic data.
...
function(config,locale) {
	...
	init : function(sandbox) {
		this.template = jQuery("<div class='manageClassificationPlugin'>" + '<div class="classheader"><div class="header-icon icon-arrow-white-right"></div></div>' + '<div class="content"></div>' + "</div>");
		// Colors
		this.setColors();
	},
	/**
	...
	eventHandlers : {
		...
		'MapStats.SotkadataChangedEvent' : function(event) {
			// Create a new classification for thematic data, if
	 ...
	 *
	 * @param event  Data sent by 'MapStats.SotkadataChangedEvent' (eg.  ManageStatsOut)
	 */
	classifyData : function() {
		...
		// Get class count
		var classes = Number(jQuery('.manageClassificationPlugin').find('.classificationMethod').find('.classCount').find('input').val());
	...
	setColors : function() {
		...
		this.colorsets = JSON.parse(colorsjson);

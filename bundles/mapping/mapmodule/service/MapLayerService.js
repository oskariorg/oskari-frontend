...
import { parseLayerListing, parserRegister } from './MapLayerParserRegister';

export class MapLayerService {
    constructor (sandbox) {
        this._sandbox = sandbox;
        this._layers = {};
        this._availableLayersLoaded = false;
        this._layerGroups = [];
    }

    async loadAvailableLayersForUser (options = {}) {
        const url = Oskari.urls.getRoute('LayerList', {
            lang: Oskari.getLang(),
            ...options
        });
        ...
        if (!response.ok) {
            throw new Error('Layers could not be loaded');
        }
        const data = await response.json();
        const { groups, layersById, providers } = parseLayerListing(data);

        // we don't want to reset "_layerGroups" at the beginning since there groups
        //  created at runtime like one for statistical regionsets and we don't want to remove those.
        this.addLayerGroups(groups);
        this.addDataProviders(providers);
        this.addLayers(layersById);

        // notify components of added layers
        this._availableLayersLoaded = true;
        this._sandbox.notifyAll(Oskari.eventBuilder('MapLayerEvent')(null, 'add'));
        return this.getLayers().length;
    }

    addLayers (layersById) {
        Object.values(layersById).forEach(layer => {
            const id = layer.getId();
            if (!this._layers[id]) {
                this._layers[id] = layer;
            } else {
                Oskari.log('MapLayerService').warn('Duplicated id for', id, layer);
            }
        });
    }

    addLayerGroups (newGroupsList) {
        this._layerGroup = [...this.getLayerGroups(), ...newGroupsList];
    }
    ...
    addDataProviders (newProvidersList) {
        this._dataProviders = [...this.getDataProviders(), ...newProvidersList];
    }
    ...
    function (sandbox) {
        ...
        loadAllLayerGroupsAjax: function (callbackSuccess, callbackFailure, options = {}) {
            ...
            jQuery.ajax({
                ...
                dataType: 'json',
                data: queryData,
                url: Oskari.urls.getRoute(options?.route || 'GetHierarchicalMapLayerGroups'),
        ...
        _loadAllLayerGroupsAjaxCallBack: function (pResp, callbackSuccess) {
            const { groups = [], layers = [], providers = {} } = pResp;
            // we don't want to reset "this._layerGroups" at the beginning since there groups
            //  created at runtime like one for statistical regionsets and we don't want to remove those.
            const groupModels = groups.map(group => Oskari.clazz.create('Oskari.mapframework.domain.MaplayerGroup', group));
            this._layerGroups.push(...groupModels);

            const parsedProviders = Object.keys(providers).map(id => {
                return {
                    id: Number(id),
            ...
            this.setDataProviders([...this.getDataProviders(), ...parsedProviders]);
            ...
            this._loadLayersRecursive(layers, () => {
        ...
        */
        ...
        createMapLayer: function (mapLayerJson) {
            const mapLayer = this._createActualMapLayer(mapLayerJson);
            // TODO: removal of layers of type 'collection' from db
            /*

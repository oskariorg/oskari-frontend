...
const PlayPauseIconSvg = ({ initial }) => {
    const [status, setStatus] = React.useState(!initial);
    const initialD = status ? PLAYPATH : PAUSEPATH;
    const [d, setD] = React.useState(initialD);
    ...
    return (
        <svg id='play-svg' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32' onClick={changeState}>
            <path d={d}>
                <animate from={from} to={to} attributeName='d' fill='freeze' dur={dur} calcMode='spline' keySplines='0.19 1 0.22 1'/>
            </path>
        </svg>
    );
};
PlayPauseIconSvg.propTypes = {

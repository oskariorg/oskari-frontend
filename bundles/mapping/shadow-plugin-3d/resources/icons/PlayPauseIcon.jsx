...
export const PlayPauseIcon = ({ initial }) => {
    ...
    const setPause = (d, from, to) => {
        ...
        setTo(to);
        if (moi.current) {
            moi.current.beginElement();
        }
        //document.getElementById('play-svg').getElementsByTagName('animate')[0]
    };
    const changeState = () => {
    ...
    return (
        <svg xmlns='http://www.w3.org/2000/svg' viewBox='-4 -4 40 40' onClick={changeState}>
            <path d={d}>
                <animate ref={moi} from={from} to={to} attributeName='d' fill='freeze' dur={dur} calcMode='spline' keySplines='0.19 1 0.22 1'/>
            </path>
        </svg>
    );
};

...
function(instance, localization) {
    ...
    initialFeedbackLoad: function(feedback) {
        var panel = this.tabPanels[this.layerId];
        if(!panel) {
            panel = this._createFeedbackTab(this.loc.title);
            this.tabsContainer.addPanel(panel);
            this.tabPanels[this.layerId] = panel;
        }
        this._populateFeedbacks(this.layerId, feedback);
    ...
    _deletePlace : function(data) {
        ...
        okBtn.setHandler(function() {
            ...
            service.deleteFeedback(data.id, callback);
        });
     ...
     */
    _createFeedbackTab : function(title) {
        var me = this;
        ...
        var visibleFields = ['name', 'description', 'createDate', 'edit', 'delete'];
        ...
        var nameRenderer = function(name, data) {
            ...
            linkIcon.addClass('feedback-' + shape);
    ...
    _populateFeedbacks : function(panelName, feedback) {
        ...
        for(var i = 0; i < feedback.length; ++i) {
            gridModel.addData({
                'id': feedback[i].attributes['ID'],
                'name' : feedback[i].attributes['NIMI'],
                'description' : feedback[i].attributes['SELITE'],
                'geometry' : feedback[i].geometry,
                'edit' : this.loc['edit'],
                'delete' : this.loc['delete'],
                'createDate' : this._formatDate(service, feedback[i].attributes['MUOKKAUS_PVM']),
            });
        }
    },

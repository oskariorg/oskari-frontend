...
(function() {
	...
	Oskari.clazz.define('Oskari.poc.all.Main', function() {
		...
		appSetup : {
			...
			sv : '?',
			en : '?',
			bundlename : 'dojo-layerselection',
			bundleinstancename : 'dojo-layerselection',
			metadata : {
			"Import-Bundle" : {
			"dojo-layerselection" : {
			bundlePath : 'bundle/'
			...
			bundleinstancename : 'featureinfo',
			metadata : {
			"Import-Bundle" : {
			"featureinfo" : {
			bundlePath : '../dojo/bundle/'
			...
			*/
			...
			sv : '?',
			en : '?',
			bundlename : 'jqueryui-layerselection',
			bundleinstancename : 'jqueryui-layerselection',
			metadata : {
			"Import-Bundle" : {
			"jqueryui-layerselection" : {
			bundlePath : 'bundle/'
			},
			"style" : {
			bundlePath : '../../proof-of-concepts/jqueryui/bundle/'
			}
			},
			"Require-Bundle-Instance" : [ 'style' ]
			},
			instanceProps : {}
			},*/
			/** KendoUI */
			...
			sv : '?',
			en : '?',
			bundlename : 'kendoui-layerselection',
			bundleinstancename : 'kendoui-layerselection',
			metadata : {
			"Import-Bundle" : {
			"kendoui-layerselection" : {
			bundlePath : 'bundle/'
			}
			},
			"Require-Bundle-Instance" : []
			...
			*/
			 ...
			 },*/
			{
				...
				metadata : {
					"Import-Bundle" : {
						"featureinfo" : {
							bundlePath : '../../proof-of-concepts/jquery/bundle/'
						}
					},
					"Require-Bundle-Instance" : []
				},
			...
			/** ExtJS support */
			{
				...
				title : 'Oskari DIV Manager and ExtJS adapter',
				fi : 'Oskari DIV Manager and ExtJS adapter',
				sv : '?',
				en : 'Oskari DIV Manager and ExtJS adapter',
				bundlename : 'flyoutadapter',
				bundleinstancename : 'flyoutadapter',
				metadata : {
					"Import-Bundle" : {
						"flyoutadapter" : {
							bundlePath : '../../proof-of-concepts/extjs/bundle/'
						}
					},
					"Require-Bundle-Instance" : []
				},
			 ...
			 },*/
			{
				callback : function() {
					...
					// FIXME: this is just for debug to get the map in
					// 		correct position and taustakartta initially selected
					Oskari.$().sandbox.getMap().moveTo('397898', '6673381', 9);
					Oskari.$().sandbox.syncMapState();
					Oskari.$().sandbox.request("AllLayersModule", Oskari.$().sandbox.getRequestBuilder('AddMapLayerRequest')("base_35", true));

					Oskari.$().sandbox.request("AllLayersModule", Oskari.$().sandbox.getRequestBuilder('AddMapLayerRequest')("15", true));

				},
				title : 'All Layers Bundle',
				fi : 'All Layers Bundle',
				sv : '?',
				en : 'All Layers Bundle',
	 ...
	 * Demo only
	 */
	Oskari.clazz.category('Oskari.poc.all.Main', 'demo', {
	...
	require(["dojo/domReady!"], function() {
		...
		Oskari.$("Ext", Ext);
		Oskari.$("startup", { imageLocation: '../../resources"'});
		
		if(args.oskariLoaderAsync && args.oskariLoaderAsync == 'on') {
			Oskari.setSupportBundleAsync(true);
		}

		var main = Oskari.clazz.create('Oskari.poc.all.Main');

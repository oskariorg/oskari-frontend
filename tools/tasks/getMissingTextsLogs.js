...
module.exports = function (grunt) {
    ...
    grunt.registerMultiTask(
        ...
        'Generate missing localization excel file for given language',
        function () {
            var locale = grunt.option('locale'),
                minifier = grunt.option('minifier'),
                locales,
                path = require('path'),
            ...
            if(!minifier){
                grunt.fail.fatal('minifier parameter is missing --> ');
            }

            var parser = require('../parser.js'),
                idt = parser.getBundles(minifier,grunt),
                langfiles = {};

            grunt.log.writeln('');
            grunt.log.writeln('');
            grunt.log.writeln('=========== Bundles to be generetad from >',minifier,'< Minifier file');
            grunt.log.writeln('=========== IDT: ', idt);

            for(var k = 0; k <langfiles.length; ++k){
                grunt.log.writeln('lang', langfiles[k]);
            }
...
//                grunt.log.writeln('files:',files[d]);
//                d += 1
                ...
                file.src.map(function (filepath) {
                    ...
                    grunt.log.writeln('');
                    grunt.log.writeln('');
                    grunt.log.writeln('These localization excels should be generated');


                    for(i = 0; i < locales.length; i += 1){
                        for (p = 0 ; p < idt.length ; p += 1) {
                            ...
                            if(bundleName == idt[p]){
                                ...
                                grunt.config.set(
                                    'generate-emptyl10n-excel-log.' + bundleName + '_' + locales[i],
                                    [{
                                ...
                                grunt.log.writeln('bundleName: ' , bundleName, ' bundleDir: ', fp, ' templateLocale: ', templateLocale, ' locale: ', locales[i]);
                            }
                        }
                    }
                });
            ...
            grunt.log.writeln('KKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK');
            for (var i = 0; i < arr.length; i++) {
                grunt.log.writeln(arr[i]);
            }

            //grunt.task.run('generate-l10n-excel-log');
            grunt.task.run('generate-emptyl10n-excel-log');
        }
    );
    ...
    grunt.registerMultiTask(
        ...
        function () {
            var done = this.async(),
                ...
                output,
                asyncCounter = 4,
                hasRows = false; // decremented on async done
            ...
            grunt.log.writeln('bundleName',bundleName);
            grunt.log.writeln('bundleDir',bundleDir);
            grunt.log.writeln('locale',locale);
            grunt.log.writeln('templateLocale',templateLocale);
            grunt.log.writeln('');
            ...
            var rowIndex = 2,
                ...
                addExcelRow = function (path, filename, key, value, translation, notes) {
                    ...
                    grunt.log.writeln('Adding excel row');
                    ...
                    //if(!translation){
                    //    grunt.log.writeln('no translation  --> adding value to translation field');
                ...
                printNodePath = function (node, stack) {
                    ...
                    if (typeof node === 'string' || node instanceof String) {
                        ...
                        if(!note) {
                            ...
                            if (note) {
                                textMissing = 'true';
                                //grunt.log.writeln(localizationDir + path.sep + bundleName + '_' + locale + '.xlsx ------',  global.savfile);
                ...
                writeExcelFile = function () {
                    ...
                    grunt.log.writeln('Writing a excel file');
            ...
            grunt.log.writeln(' ====  DONE with: ', bundleName);
            grunt.log.writeln('');
        }

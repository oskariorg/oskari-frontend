...
'Oskari.mapframework.core.Core', 'feature-info-methods',
{
    handleGetFeatureInfoRequest : function(request) {
        ...
        if (request.getMapLayers().length > 0 && !wfsSelected) {
            /* notify sandbox */
            var event = this.getEventBuilder('AfterGetFeatureInfoEvent')(true, false);
            ...
            for (var i = 0; i < request.getMapLayers().length; i++) {
                ...
                if (layer.getQueryable() == true && format != "" && format != null && !(layer.isLayerOfType('WFS'))) {
                    ...
                    var wmsBoundingBox = request.getBoundingBox().left + ',' + request.getBoundingBox().bottom + ',' + request.getBoundingBox().right + ',' + request.getBoundingBox().top;

                    //console.log(layer.getCurrentStyle().getName());
                    //console.dir(layer.getCurrentStyle());
                    // TODO: is this really the way we want to do this?
                    url = url + 'REQUEST=GetFeatureInfo' + '&EXCEPTIONS=application/vnd.ogc.se_xml' + '&SRS=' + request.getSRS() + '&VERSION=1.1.1' + '&BBOX=' + wmsBoundingBox + '&X=' + request.getX() + '&Y=' + request.getY() + '&INFO_FORMAT=' + format + '&QUERY_LAYERS=' + layer.getWmsName() + '&WIDTH=' + request.getMapWidth() + '&HEIGHT=' + request.getMapHeight() + '&FEATURE_COUNT=1' + '&FORMAT=image/png' + '&SERVICE=WMS' + '&STYLES=' + layer.getCurrentStyle().getName() + '&LAYERS=' + layer.getWmsName();
                    /* example url:
                    ...
                    */
                    ...
                    //var mapLayer = openLayersMap.getLayersByName('layer_' +
                    // layer.getId());
                    ...
                    var callbackTemplate = function(name, format) {
                        ...
                        this.parseOkMessage = function(response) {
                            var parsedMessage = this.parseInfoResponse(response, this._format);

                            var event = this.getEventBuilder('AfterAppendFeatureInfoEvent')(name, parsedMessage);
                            this.copyObjectCreatorToFrom(event, request);
                            this.dispatch(event);
                        };
                        ...
                        this.parseFailedMessage = function(response) {
                            ...
                            var event = this.getEventBuilder('AfterAppendFeatureInfoEvent')(name, response.responseText);
                            this.copyObjectCreatorToFrom(event, request);
                            this.dispatch(event);
                        };
                    };

                    var callback = new callbackTemplate(layerName, format);
                    ...
                    var errorMessage = "";
                    errorMessage = this.getText("rightpanel_wms_getfeatureinfo_not_supported_txt") + ": " + layerName + ".";
                    var event = this.getEventBuilder('AfterAppendFeatureInfoEvent')(layerName, errorMessage);
                    this.copyObjectCreatorToFrom(event, request);
                    this.dispatch(event);
                }
            ...
            this.parseInfoResponse = function(response, format) {
                ...
                var parsedResp;
                if (format == 'application/vnd.ogc.gml' || format == 'application/vnd.ogc.se_xml') {
                    parsedResp = this.renderInfoGML(response);
                } else if (format == 'application/vnd.ogc.wms_xml' || format == 'text/xml') {
                    ...
                    if (response.responseText == '') {
                        parsedResp = this.getText('mapcontrolsservice_not_found_wms_feature_info');
                    }
                }
            ...
            this.renderInfoGML = function(response) {
                return response.responseText.replace(/</g, '&lt;').replace(/>/g, '&gt;');
            };
            ...
            var event = this.getEventBuilder('AfterGetFeatureInfoEvent')(false, wfsSelected);
            this.copyObjectCreatorToFrom(event, request);
            this.dispatch(event);
        }
    }
